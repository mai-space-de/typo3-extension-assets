...
final class JsViewHelper extends AbstractViewHelper
{
    ...
    public function initializeArguments(): void
    {
        ...
        $this->registerArgument(
            "src",
            "string",
            "EXT: path or absolute path to JS file",
            true,
        );
        $this->registerArgument(
            "priority",
            "bool",
            "<head> (true) vs. footer (false)",
            false,
            false,
        );
        $this->registerArgument(
        ...
        $this->registerArgument(
            "async",
            "bool",
            "async attribute (mutually exclusive with defer)",
            false,
            false,
        );
        $this->registerArgument(
            "type",
            "string",
            'MIME type; set "module" for ES6 modules',
            false,
            "",
        );
        $this->registerArgument(
        ...
        $this->registerArgument(
            "integrity",
            "string",
            "SRI hash; auto-computed for local files",
            false,
            "",
        );
        $this->registerArgument(
    ...
    public function render(): string
    {
        ...
        if ($minify) {
            ...
            $cacheDir = \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName(
